...
async function fileToCompressedJpegBase64(
  file: File,
  maxBytes = 240 * 1024, // margem de segurança
): Promise<string> {
  ...
  // Tenta diferentes qualidades até caber no limite
  ...
  // Fallback — retorna a menor qualidade mesmo assim
...
export default function PlaylistHeader({
  ...
  async function handleCoverFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      // Atualiza preview localmente — Spotify pode demorar a propagar a URL real
      ...
      <div className="relative z-10 flex flex-col justify-end px-4 pb-6 pt-8 sm:px-8 sm:pb-8 sm:pt-12 laptop:px-12">
        <div className="flex flex-col gap-4 sm:gap-6 laptop:flex-row laptop:items-end">
          ...
          <div className="shrink-0 self-center laptop:self-end">
            ...
            />
            {coverError && (
              <p className="mt-2 max-w-[160px] text-center text-[11px] text-red-300 sm:max-w-[160px] laptop:text-left">
                {coverError}
              </p>
            )}
          </div>
